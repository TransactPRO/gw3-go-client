...
func TestParseRecurrentSMSSuccessfulRedirect(t *testing.T) {
	...
	parsedResponse, err := NewRecurrentSMSAssembly().ParseResponse(response)
...
func TestParseRecurrentDMSSuccessfulRedirect(t *testing.T) {
	expectedURL, _ := url.Parse("https://api.url/a4345be5b8a1af9773b8b0642b49ff26")
	...
	response := structures.NewGatewayResponse(nil, []byte(body))
	parsedResponse, err := NewRecurrentDMSAssembly().ParseResponse(response)
	assert.NoError(t, err)
