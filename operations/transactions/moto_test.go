...
func TestParseMOTOSMSResponseSuccessfulRedirect(t *testing.T) {
	...
	parsedResponse, err := NewMOTOSMSAssembly().ParseResponse(response)
...
func TestParseMOTODMSResponseSuccessfulRedirect(t *testing.T) {
	expectedURL, _ := url.Parse("https://api.url/a4345be5b8a1af9773b8b0642b49ff26")
	...
	response := structures.NewGatewayResponse(nil, []byte(body))
	parsedResponse, err := NewMOTODMSAssembly().ParseResponse(response)
	assert.NoError(t, err)
